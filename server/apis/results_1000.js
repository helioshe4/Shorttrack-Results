...
router.post("/:skater_id", async (req, res) => {
  try {
    ...
    let query = "INSERT INTO results_1000 (skater_id";
...
router.put("/:skater_id", async (req, res) => {
  try {
    ...
    let query = "UPDATE results_1000 SET";
    const values = [];
    const params = [];
...
router.get("/:skater_id", async (req, res) => {
  try {
    ...
    const result_1000 = await pool.query(
      "SELECT * FROM results_1000 WHERE result_id = $1",
      [result_id]
    );

    res.json(result_1000.rows[0]);
  } catch (err) {
    console.error(err.message);
  }
});
...
router.get("/", async (req, res) => {
  try {
    const allResult_1000 = await pool.query("SELECT * FROM results_1000;");

    res.json(allResult_1000.rows);
  } catch (err) {
    console.error(err.message);
  }
});
...
//gets results_1000 given skater name
...
router.get("/result-id/:skater_id", async (req, res) => {
  try {
    ...
    const result_id = await pool.query(
      "SELECT result_id FROM results_1000 WHERE skater_id = $1 LIMIT 1",
      [skater_id]
    );
